...
def start_game(nickname):
    WSize = (720, 480)
    win_game = pygame.display.set_mode(WSize)
...
def main_menu():
    ...
    while True:
            ...
            if e.type == QUIT:
                ...
                for i, (_, position) in enumerate(menu_positions):
                    if position.collidepoint(mouse_pos):
                        ...
                        if nickname.strip() or action == "quit":
                            menu_actions[action]()
        ...
        nickname_surface = font.render(nickname or "nickname", True, WHITE if nickname else (200, 200, 200))
